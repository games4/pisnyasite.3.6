<?php 
$result_composition = mysql_query ("SELECT DISTINCT vydannya.title AS vydannya, kompozycija.id, kompozycija.title, vykonavec.title AS vykonavec FROM vydannya, kompozycija, view, vykonavec WHERE vydannya.id = $id AND kompozycija.vydannya = vydannya.id AND view.kompozycija = kompozycija.id AND kompozycija.vykonavec1 = vykonavec.id ORDER BY kompozycija.title", $db);
